...
int main(int argc, char* argv[])
{
	...
	/*************************/
	int n, ri, ci, paso, maxPasos, userMax = 0, m;
	...
	for(int ren = 0; ren < n; ren++)
	{
		for(int col = 0; col < n; col++)
		{
			...
			{
				//calif = 0;
				cerr << "Dio más pasos que nuestro programa! " << paso << endl;
				maxPasos = paso;
				//break;
			}
	...
	//Checar que de el máximo de pasos
	if(userMax < maxPasos) {
		cerr << "No dio el recorrido más largo" << endl;
		calif = 0;
	}
	...
	for(int I = 1; I <= maxPasos; I++)
	{
		...
		//Checo que cada paso sea a una casilla de buena suerte
		if((leidos[I][1]+leidos[I][2]+2)%m == 0)
		{
			cerr << "Caballo dio paso " << I << " de mala suerte" << endl;
			calif = 0;
			break;
		}
	}
	...
	//Checo que ninguno de los 8 vecinos sean 0 de buena suerte
	int posx = leidos[maxPasos][1];
	int posy = leidos[maxPasos][2];

	for (int i = 0; i < 8; ++i) {
        posx += vecinos[i][0];
        posy += vecinos[i][1];
        if (posx >= 0 && posx < n && posy >= 0 && posy < n) {
            if (tablero[posx][posy] == 0 && (posx + posy + 2)%m != 0) {
