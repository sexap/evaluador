...
unsigned int A[21][21], B[21][21], Apasos=0;
unsigned int Cflojo(int Ncaballo,int N,int R,int C,int pasos, int M)
{
	if (Ncaballo<=(N*N) /*&& pasos>Apasos*/)
	{
		if (((1<=C)&&(C<=N))&&((1<=R)&&(R<=N)))
		{
			...
			for(k=0;k<=8;k++)
			{
				...
				{
				case 0:
					if(A[R-2][C-1]==0 && (((R-2)+(C-1))%M !=0))
					{
				
					Cflojo(Ncaballo+1,N,R-2,C-1,pasos+1,M);
					
					}
					break;
				case 1:
					if(A[R-1][C-2]==0 && (((R-1)+(C-2))%M !=0))
					{
				
					Cflojo(Ncaballo+1,N,R-1,C-2,pasos+1,M);
